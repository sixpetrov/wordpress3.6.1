...
add_action('media_buttons', 'iphorm_add_insert_button', 20);  //WPSP
...
{
    global $wpdb;
    $tableName = iphorm_get_form_entries_table_name();

    if ($unread !== null) {
        $unread_count = absint($unread);
        return $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM %s WHERE unread = %d", $tableName, $unread_count));
    } else {
        return $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM %s", $tableName));
    }
}
