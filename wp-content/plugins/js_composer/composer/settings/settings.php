...
class WPBakeryVisualComposerSettings {
    ...
    public function __construct($composer) {
        ...
        if(WPBakeryVisualComposer::getInstance()->isPlugin()) {
            ...
            //$this->tabs['updater'] = __('Updater', 'js_composer'); //wpsp_change

            $this->tabs['custom_css_classes'] = "CSS classes";
        }
    ...
    public function init() {
         ...
         * CSS CLASSES
         */
        $tab_prefix = '_custom_css_classes';
        register_setting($this->option_group.$tab_prefix, self::$field_prefix.'custom_css_classes', array($this, 'sanitize_custom_css_callback'));
        add_settings_section($this->option_group.$tab_prefix,
            null,
            array(&$this, 'setting_section_callback_function'),
            $this->page.$tab_prefix);
        add_settings_field(self::$field_prefix.'custom_css_classes', __("Insert CSS classes here", "js_composer"), array(&$this, 'custom_css_classes_callback'), $this->page.$tab_prefix, $this->option_group.$tab_prefix);
    }
    /**
     * Creates new section.
...
* @param $callback - function to build section header.
    ...
    //wpsp_add
    public function custom_css_classes_callback() {
        $value = ($value = get_option(self::$field_prefix.'custom_css_classes')) ? $value : 'Bottom margin: vc_bottom_margin';
        echo '<textarea style="width:635px;height:177px;" name="'.self::$field_prefix.'custom_css_classes'.'" class="custom_css_classes">'.$value.'</textarea>';
        echo '<p class="description indicator-hint">'.__("Each custom CSS class has to be on new line. Class name: css_class", "js_composer").'</p>';
    }
    /**
