...
function vcsp_trim_text($content, $amount)
{
...
$grid_content = $el_class = $width = $orderby = $order = $el_position = $isotope_item = $excerpt_amount = $grid_read_more = '';
...
$output = '';
$grid_read_more = trim($grid_read_more);
...
while ( $my_query->have_posts() ) {
    ...
    if($grid_content == 'teaser') {
        ...
        $excerpt = vcsp_trim_text($excerpt, $excerpt_amount);
        $content = apply_filters('the_excerpt', $excerpt);
    } else {
    ...
    // Read more link
    if ( ! empty($grid_read_more) ) {
        $link = '<a class="more-link" href="'. get_permalink($post_id) .'"'.$link_target.' title="'. sprintf( esc_attr__( 'Permalink to %s', 'js_composer' ), the_title_attribute( 'echo=0' ) ).'">'. $grid_read_more .'</a>';
    }
