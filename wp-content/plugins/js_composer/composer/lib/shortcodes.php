        ...
        protected function singleParamEditForm($param, $param_value) {
            ...
            // Checkboxes with post types
                ...
                foreach ( $values as $label => $v ) {
                    ...
                    //--wpsp_change
                    $param_line .= ' <div style="margin-bottom:5px"><input id="'. $param['param_name'] . '-' . $v .'" value="' . $v . '" class="'.$param['param_name'].' '.$param['type'].'" type="checkbox" name="'.$param['param_name'].'"'.$checked.'> ' . "<label for='". $param['param_name'] ."-". $v ."'>$label</label></div>";
                }
            }
