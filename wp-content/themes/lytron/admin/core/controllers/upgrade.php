...
*  Upgrade
*
*  @description: All the functionality for upgrading aki
*  @since 3.2.6
*  @created: 23/06/12
*/
...
class aki_upgrade
{
	...
	*/
	...
	function __construct($parent)
	{
		...
		add_action('wp_ajax_aki_upgrade', array($this, 'upgrade_ajax'));
	...
	*/
	...
	function admin_menu()
	{
		add_submenu_page('edit.php?post_type=aki', __('Upgrade','aki'), __('Upgrade','aki'), 'manage_options','aki-upgrade', array($this,'html') );
	}
	...
	*/
	...
	function init()
	{
		$version = get_option('aki_version', false);
		if( $version )
		{
			if( $version < $this->parent->upgrade_version )
			{
				$this->parent->admin_message('<p>' . __("Advanced Custom Fields",'aki') . ' v' . $this->parent->version . ' ' . __("requires a database upgrade",'aki') .' (<a class="thickbox" href="' . admin_url() . 'plugin-install.php?tab=plugin-information&plugin=advanced-custom-fields&section=changelog&TB_iframe=true&width=640&height=559">' . __("why?",'aki') .'</a>). ' . __("Please",'aki') .' <a href="http://codex.wordpress.org/Backing_Up_Your_Database">' . __("backup your database",'aki') .'</a>, '. __("then click",'aki') . ' <a href="' . admin_url() . 'edit.php?post_type=aki&page=aki-upgrade" class="button">' . __("Upgrade Database",'aki') . '</a></p>');
				
			}
			elseif( $version < $this->parent->version)
			{
				update_option('aki_version', $this->parent->version );
			}
		}
		else
		{
	...
	*/
	...
	function html()
	{
		$version = get_option('aki_version','1.0.5');
		$next = false;
					...
					success: function(json){
						...
						if(json)
						{
							if(json.status)
							{
								...
								{
									// all done
									add_message('Upgrade Complete! <a href="<?php echo admin_url(); ?>edit.php?post_type=aki">Continue to aki &raquo;</a>');
								}
							}
							else
							{
	...
	*/
	...
	function upgrade_ajax()
	{
		...
		// tables
		$aki_fields = $wpdb->prefix.'aki_fields';
		$aki_values = $wpdb->prefix.'aki_values';
		$aki_rules = $wpdb->prefix.'aki_rules';
		...
		{	
			...
			*--------------------*/
			...
			case '3.0.0':
				...
				if($akis)
				{
					...
					{
						// position
						update_post_meta($aki->ID, 'position', 'normal');
						
						//layout
						$layout = get_post_meta($aki->ID, 'field_group_layout', true) ? get_post_meta($aki->ID, 'field_group_layout', true) : 'in_box';
						if($layout == 'in_box')
						{
							$layout = 'default';
						}
						...
						update_post_meta($aki->ID, 'layout', $layout);
						delete_post_meta($aki->ID, 'field_group_layout');
						
						// show_on_page
						$show_on_page = get_post_meta($aki->ID, 'show_on_page', true) ? get_post_meta($aki->ID, 'show_on_page', true) : array();
						if($show_on_page)
				 		{
				 			$show_on_page = unserialize($show_on_page);
				 		}
				 		update_post_meta($aki->ID, 'show_on_page', $show_on_page);
				...
				if($akis)
				{
					foreach($akis as $aki)
					{
						// allorany doesn't need to change!
			 			...
			 			$rules = $wpdb->get_results("SELECT * FROM $aki_rules WHERE aki_id = '$aki->ID' ORDER BY order_no ASC", ARRAY_A);
						...
						if($rules)
						{
							foreach($rules as $rule)
							{
								...
								add_post_meta($aki->ID, 'rule', $rule);
							}
						}
			...
			*--------------------*/
			...
			case '3.0.0 (step 3)':
			    ...
			    $parent_id = 0;
			    $fields = $wpdb->get_results("SELECT * FROM $aki_fields WHERE parent_id = $parent_id ORDER BY order_no, name", ARRAY_A);
				 	...
				 	foreach($fields as $field)
				 	{
						...
						else
						{
				 		...
				 		if($field['type'] == 'repeater')
				 		{
				 			...
				 			$parent_id = $field['id'];
				 			$sub_fields = $wpdb->get_results("SELECT * FROM $aki_fields WHERE parent_id = $parent_id ORDER BY order_no, name", ARRAY_A);
					
				 			
				 			// if fields are empty, this must be a new or broken aki.
						 	if(empty($sub_fields))
						 	{
						 		$field['options']['sub_fields'] = array();
						 	}
			...
			*--------------------*/
			...
			case '3.0.0 (step 4)':
				...
				$values = $wpdb->get_results("SELECT v.field_id, m.post_id, m.meta_key, m.meta_value FROM $aki_values v LEFT JOIN $wp_postmeta m ON v.value = m.meta_id WHERE v.sub_field_id = 0", ARRAY_A);
				if($values)
				{
				...
				$values = $wpdb->get_results("SELECT v.field_id, v.sub_field_id, v.order_no, m.post_id, m.meta_key, m.meta_value FROM $aki_values v LEFT JOIN $wp_postmeta m ON v.value = m.meta_id WHERE v.sub_field_id != 0", ARRAY_A);
				if($values)
				{
					$rows = array();
					...
					foreach($values as $value)
					{
						...
						// get field name
						$field_name = $wpdb->get_var($wpdb->prepare("SELECT name FROM $aki_fields WHERE id = %d", $value['field_id']));
						
						// get sub field name
						$sub_field_name = $wpdb->get_var($wpdb->prepare("SELECT name FROM $aki_fields WHERE id = %d", $value['sub_field_id']));
					...
					foreach($rows as $post_id => $field_ids)
					{
						...
						{
							// get sub field name
							$field_name = $wpdb->get_var($wpdb->prepare("SELECT name FROM $aki_fields WHERE id = %d", $field_id));
				...
				update_option('aki_version','3.0.0');
			...
			*--------------------*/
			...
			case '3.1.8':
				...
				// vars
				$message = __("Migrating options values from the $wp_postmeta table to the $wp_options table",'aki') . '...';
				...
				// update version
				update_option('aki_version','3.1.8');
			...
			*--------------------*/
			...
			case '3.2.5':
				...
				// vars
				$message = __("Modifying field group options 'show on page'",'aki') . '...';
				...
				if($akis)
				{
					foreach($akis as $aki)
					{
						...
						update_post_meta($aki->ID, 'hide_on_screen', $hide_on_screen);
						delete_post_meta($aki->ID, 'show_on_page');
				...
				// update version
				update_option('aki_version','3.2.5');
			...
			*/
			...
			case '3.3.3':
				...
				// vars
				$message = __("Modifying field option 'taxonomy'",'aki') . '...';
				...
				foreach($akis as $aki)
				{
					...
					foreach( $fields as $field )
					{
						...
						$this->parent->update_field( $aki->ID, $field);
				...
				// update version
				update_option('aki_version','3.3.3');
			...
			*/
			...
			case '3.4.1':
				...
				// vars
				$message = __("Moving user custom fields from wp_options to wp_usermeta'",'aki') . '...';
				...
				if( $option_rows )
				{
					foreach( $option_rows as $k => $row)
					{
						...
						// if no matches, this is not an aki value, ignore it
						if( !$matches )
						{
							continue;
						}
				...
				// update version
				update_option('aki_version','3.4.1');
