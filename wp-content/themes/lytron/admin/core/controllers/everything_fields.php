...
class aki_everything_fields
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
		...
		add_action('wp_ajax_aki_everything_fields', array($this, 'aki_everything_fields'));
	...
	*-------------------------------------------------------------------------------------*/
	...
	function admin_menu() 
	{
		...
		// get field groups
		$metabox_ids = array();
		$this->data['metabox_ids'] = apply_filters( 'aki/location/match_field_groups', $metabox_ids, $options );
...
?>
					...
					setTimeout( function(){ 
						$(document).trigger('aki/setup_fields', $('#wpbody') );
					}, 200);
	...
	*-------------------------------------------------------------------------------------*/
	...
	function aki_everything_fields()
	{
		// defaults
		$defaults = array(
			'metabox_ids' => '',
		...
		if($akis)
		{
			foreach($akis as $aki)
			{
				...
				if( !in_array( $aki['id'], $options['metabox_ids'] ) )
				{
					continue;
				}
				...
				$title = "";
				if ( is_numeric( $aki['id'] ) )
			    {
			        $title = get_the_title( $aki['id'] );
			    }
			    else
			    {
			        $title = apply_filters( 'the_title', $aki['title'] );
			    }
				...
				{
					echo '<div class="postbox aki_postbox" id="aki_'. $aki['id'] .'">';
				...
				foreach($aki['fields'] as $field)
				{
					...
					if( $layout == 'metabox' )
					{
						echo '<div id="aki-' . $field['name'] . '" class="field field-' . $field['type'] . ' field-'.$field['key'] . $required_class . '">';
					...
					elseif( $layout == 'div' )
					{
						echo '<div id="aki-' . $field['name'] . '" class="form-field field field-' . $field['type'] . ' field-'.$field['key'] . $required_class . '">';
							echo '<label for="fields[' . $field['key'] . ']">' . $field['label'] . $required_label . '</label>';	
							$field['name'] = 'fields[' . $field['key'] . ']';
							do_action('aki/create_field', $field );
					...
					{
						echo '<tr id="aki-' . $field['name'] . '" class="form-field field field-' . $field['type'] . ' field-'.$field['key'] . $required_class . '">';
