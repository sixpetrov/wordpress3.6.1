...
class aki_Wysiwyg extends aki_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
    	...
    	$this->name = 'wysiwyg';
		$this->title = __("Wysiwyg Editor",'aki');
		
		add_action( 'aki_head-input', array( $this, 'aki_head') );
		
		add_filter( 'aki/fields/wysiwyg/toolbars', array( $this, 'toolbars'), 1, 1 );
   	...
   	*/
   	...
   	function toolbars( $toolbars )
   	{
   		...
   		// Custom - can be added with aki/fields/wysiwyg/toolbars filter
	...
	*-------------------------------------------------------------------------------------*/
	...
	{	
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label><?php _e("Toolbar",'aki'); ?></label>
			</td>
			<td>
				<?php
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label><?php _e("Show Media Upload Buttons?",'aki'); ?></label>
			</td>
			<td>
				<?php 
				do_action('aki/create_field', array(
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label><?php _e("Run filter \"the_content\"?",'aki'); ?></label>
				<p class="description"><?php _e("Enable this filter to use shortcodes within the WYSIWYG field",'aki'); ?></p>
				<p class="description"><?php _e("Disable this filter if you encounter recursive template problems with plugins / themes",'aki'); ?></p>
			</td>
			<td>
				<?php 
				do_action('aki/create_field', array(
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		?>
		<div id="wp-<?php echo $id; ?>-wrap" class="aki_wysiwyg wp-editor-wrap" data-toolbar="<?php echo $field['toolbar']; ?>" data-upload="<?php echo $field['media_upload']; ?>">
