...
/*
 *	This is the base aki field frow which
...
class aki_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function update_value($post_id, $field, $value)
	{
		...
		// apply filters
		$value = apply_filters('aki_update_value', $value, $field, $post_id );
		...
		foreach( $keys as $key )
		{
			if( isset($field[ $key ]) )
			{
				$value = apply_filters('aki_update_value-' . $field[ $key ], $value, $field, $post_id);
			}
		}
		...
		wp_cache_delete('aki_get_field_' . $post_id . '_' . $field['name']);
	...
	*-------------------------------------------------------------------------------------*/
	...
	function get_value($post_id, $field)
	{
		...
		// apply filters
		$value = apply_filters('aki_load_value', $value, $field, $post_id );
		...
		foreach( $keys as $key )
		{
			if( isset($field[ $key ]) )
			{
				$value = apply_filters('aki_load_value-' . $field[ $key ], $value, $field, $post_id);
			}
		}
