...
class aki_Relationship extends aki_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
		...
		$this->title = __("Relationship",'aki');
		
		
		// actions
		add_action('wp_ajax_aki_get_relationship_results', array($this, 'aki_get_relationship_results'));
	...
	*-------------------------------------------------------------------------------------*/
   	...
   	function aki_get_relationship_results()
   	{
   		// vars
		$options = array(
			'post_type'	=>	'',
		...
		// filters
		$options = apply_filters('aki/fields/relationship/query', $options);
		$options = apply_filters('aki/fields/relationship/query-' . $options['field_name'] , $options);
		$options = apply_filters('aki/fields/relationship/query-' . $options['field_key'], $options);
		
		
		$results = '';
		$results = apply_filters('aki/fields/relationship/results', $results, $options);
		$results = apply_filters('aki/fields/relationship/results-' . $options['field_name'] , $results, $options);
		$results = apply_filters('aki/fields/relationship/results-' . $options['field_key'], $results, $options);
		...
		if( ! $results )
		{
			...
			if( $posts )
			{
				foreach( $posts  as $post )
				{
					...
					// filters
					$title = apply_filters('aki/fields/relationship/result', $title, $post);
					$title = apply_filters('aki/fields/relationship/result-' . $options['field_name'] , $title, $post);
					$title = apply_filters('aki/fields/relationship/result-' . $options['field_key'], $title, $post);

					
					$results .= '<li><a href="' . get_permalink($post->ID) . '" data-post_id="' . $post->ID . '">' . $title .  '<span class="aki-button-add"></span></a></li>';
				}
			}
		}
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		?>
<div class="aki_relationship" data-max="<?php echo $field['max']; ?>" data-s="" data-paged="1" data-post_type="<?php echo implode(',', $field['post_type']); ?>" data-taxonomy="<?php echo implode(',', $field['taxonomy']); ?>" <?php if( defined('ICL_LANGUAGE_CODE') ){ echo 'data-lang="' . ICL_LANGUAGE_CODE . '"';} ?>>
	...
	<li>
		<a href="#" data-post_id="{post_id}">{title}<span class="aki-button-remove"></span></a>
	...
	<div class="relationship_left">
		<table class="widefat">
			<thead>
				<tr>
					<th>
						<label class="relationship_label" for="relationship_<?php echo $field['name']; ?>"><?php _e("Search",'aki'); ?>...</label>
	...
	<div class="relationship_right">
		...
		if( $field['value'] )
		{
			foreach( $field['value'] as $post )
			{
				...
				echo '<li>
					<a href="' . get_permalink($post->ID) . '" class="" data-post_id="' . $post->ID . '">' . $title . '<span class="aki-button-remove"></span></a>
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_options($key, $field)
	{
		...
		<tr class="field_option field_option_<?php echo $this->name; ?>">
			<td class="label">
				<label><?php _e("Maximum posts",'aki'); ?></label>
			</td>
			<td>
				<?php 
				do_action('aki/create_field', array(
