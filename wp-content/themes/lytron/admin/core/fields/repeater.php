...
class aki_Repeater extends aki_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		$field['value']['akicloneindex'] = array();
		foreach( $field['sub_fields'] as $sub_field)
		{
			$sub_value = isset($sub_field['default_value']) ? $sub_field['default_value'] : false;
			$field['value']['akicloneindex'][ $sub_field['key'] ] = $sub_value;
		}

?>
<div class="repeater" data-min_rows="<?php echo $field['row_min']; ?>" data-max_rows="<?php echo $field['row_limit']; ?>">
	<table class="widefat aki-input-table <?php if( $field['layout'] == 'row' ): ?>row_layout<?php endif; ?>">
	...
	<?php if( $field['value'] ): foreach( $field['value'] as $i => $value ): ?>
		...
		<tr class="<?php echo ( (string) $i == 'akicloneindex') ? "row-clone" : "row"; ?>">
		...
		if( $field['row_min'] < $field['row_limit'] ): ?>
			<td class="remove">
				<a class="aki-button-add add-row-before" href="javascript:;"></a>
				<a class="aki-button-remove" href="javascript:;"></a>
			</td>
		<?php endif; ?>
	...
	<ul class="hl clearfix repeater-footer">
		<li class="right">
			<a href="javascript:;" class="add-row-end aki-button"><?php echo $field['button_label']; ?></a>
		</li>
	</ul>
...
<tr class="field_option field_option_<?php echo $this->name; ?> field_option_<?php echo $this->name; ?>_fields">
	<td class="label">
		<label><?php _e("Repeater Fields",'aki'); ?></label>
	</td>
	<td>
	<div class="repeater">
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Minimum Rows",'aki'); ?></label>
	</td>
	<td>
		<?php 
		do_action('aki/create_field', array(
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php _e("Maximum Rows",'aki'); ?></label>
	</td>
	<td>
		<?php 
		do_action('aki/create_field', array(
