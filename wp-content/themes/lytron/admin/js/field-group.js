...
(function($){
	...
	*/
	...
	aki.sortable_helper = function(e, ui)
	{
	...
	*/
	...
	$('#submit-delete').live('click', function(){
		...
		var response = confirm(aki.text.move_to_trash);
		if(!response)
		{
			return false;
		}
	...
	/*
	*  aki/update_field_options
	*  
	*  @since			3.1.6
	*  @description		Load in the opions html
	*/
	
	$('#aki_fields tr.field_type select').live('change', function(){
		...
		{
			// add loading gif
			var tr = $('<tr"><td class="label"></td><td><div class="aki-loading"></div></td></tr>');
	...
	*/
	...
	function update_order_numbers(){
		...
		$('#aki_fields .fields').each(function(){
	...
	*/
	...
	$('#aki_fields a.aki_edit_field').live('click', function(){
		...
		if( field.hasClass('form_open') )
		{
			...
			$(document).trigger('aki/field_form-close', field);
		}
		else
		{
			field.addClass('form_open');
			$(document).trigger('aki/field_form-open', field);
		}
	...
	*/
	...
	$('#aki_fields a.aki_delete_field').live('click', function(){
	...
	*/
	...
	$('#aki_fields a.aki_duplicate_field').live('click', function(){
		...
		if( field.hasClass('form_open') )
		{
			field.find('.aki_edit_field').first().trigger('click');
		}
		else
		{
			new_field.find('.aki_edit_field').first().trigger('click');
		}
	...
	*/
	...
	$('#aki_fields #add_field').live('click',function(){
		...
		// open up form
		new_field.find('a.aki_edit_field').first().trigger('click');
	...
	*/
	...
	$('#aki_fields tr.field_label input.label').live('blur', function()
	{
		var label = $(this);
	...
	*/
	...
	$('#aki_fields .field_form tr.field_label input.label').live('keyup', function()
	{
		var val = $(this).val();
		var name = $(this).closest('.field').find('td.field_label strong a').first().html(val);
	});
	$('#aki_fields .field_form tr.field_name input.name').live('keyup', function()
	{
		var val = $(this).val();
		var name = $(this).closest('.field').find('td.field_name').first().html(val);
	});
	$('#aki_fields .field_form tr.field_type select').live('change', function()
	{
		var val = $(this).val();
	...
	// sortable
	$('#aki_fields td.field_order').live('mouseover', function(){
	...
	*/
	...
	$(document).ready(function(){
		...
		{
			var html = $('#aki_location_options_deactivated').html();
	...
	*/
	...
	$('#location_rules .param select').live('change', function(){
		...
		// add loading gif
		var div = $('<div class="aki-loading"></div>');
	...
	*/
	...
	$('#location_rules a.aki-button-add').live('click',function(){
	...
	*/
	...
	$('#location_rules a.aki-button-remove').live('click',function(){
	...
	*---------------------------------------------------------------------*/
	...
	$(document).ready(function(){
		...
		$('#submitdiv #publish').attr('class', 'aki-button');
	...
	*---------------------------------------------------------------------*/
	...
	$('#aki_fields .aki_fc_add').live('click', function(){
		...
		new_tr.find('.aki_cf_meta input[type="text"]').val('');
		...
		// display
		new_tr.find('.aki_cf_meta select').val('row').trigger('change');
	...
	*---------------------------------------------------------------------*/
	...
	$('#aki_fields .aki_fc_duplicate').live('click', function(){
	...
	*---------------------------------------------------------------------*/
	...
	$('#aki_fields .aki_fc_delete').live('click', function(){
		...
		if( tr_count <= 1 )
		{
			alert( aki.text.flexible_content_no_fields );
			return false;
		}
	...
	*---------------------------------------------------------------------*/
	...
	$('#aki_fields .aki_fc_reorder').live('mouseover', function(){
		
		var table = $(this).closest('table.aki_field_form_table');
		...
		table.addClass('sortable').children('tbody').sortable({
			...
			handle: 'a.aki_fc_reorder',
			helper: aki.sortable_helper,
	...
	*---------------------------------------------------------------------*/
	...
	$('#aki_fields .aki_fc_label input[type="text"]').live('blur', function(){
		
		var label = $(this);
		var name = $(this).parents('td').siblings('td.aki_fc_name').find('input[type="text"]');
	...
	*/
	...
	$('#aki_fields .field_option_repeater_layout input[type="radio"]').live('click', function(){
	...
	$(document).live('aki/field_form-open', function(e, field){
	...
	*/
	...
	$('#aki_fields .aki_fc_display select').live('change', function(){
	...
	$(document).live('aki/field_form-open', function(e, field){
		...
		$(field).find('.aki_fc_display select').each(function(){
			$(this).trigger('change');
		});
	...
	*/
	...
	$('#adv-settings input[name="show-field_key"]').live('change', function(){
		...
		if( $(this).val() == "1" )
		{
			$('#aki_fields table.aki').addClass('show-field_key');
		}
		else
		{
			$('#aki_fields table.aki').removeClass('show-field_key');
		}
	...
	*/
	...
	aki.create_field = function( options ){
	...
	$(document).live('aki/field_form-open', function(e, field){
		...
		$(field).find('.conditional-logic-field').each(function(){
			...
			if( aki.fields )
			{
				for( i = 0; i < aki.fields.length; i++ )
				{
					choices.push({
						value : aki.fields[i].id,
						label : aki.fields[i].label
					});
				}
			}
			...
			if( choices.length == 0 )
			{
				choices.push({
					'value' : 'null',
					'label' : aki.text.conditional_no_fields
				})
			}
	
			
			// create select
			select = aki.create_field({
				'type' : 'select',
	...
	*/
	...
	$('select.conditional-logic-field').live('change', function(){
		...
		{
			choices = [
				{ value : 1, label : aki.text.checked }
			];
	...
	*
	*  @description: populates the aki object with all available fields
	*  @since 3.5.1
	*  @created: 15/10/12
	*/
	
	aki.setup_conditional_fields = function()
	{
		aki.fields = [];
		
		$('#aki_fields > .inside > .fields > .field').each(function(){
	...
	*/
	...
	$('tr.conditional-logic .aki-button-add').live('click',function(){
	...
	*/
	...
	$('tr.conditional-logic .aki-button-remove').live('click',function(){
