...
(function($){
	...
	var _relationship = aki.fields.relationship;
	...
	*/
	...
	$(document).live('aki/setup_fields', function(e, postbox){
		...
		$(postbox).find('.aki_relationship').each(function(){
			...
			$(this).find('.relationship_left .relationship_list').scrollTop(0).scroll( function(){
				...
				// vars
				var div = $(this).closest('.aki_relationship');
	...
	*/
	...
	$('.aki_relationship .relationship_left .relationship_list a').live('click', function(){
		
		// vars
		var id = $(this).attr('data-post_id'),
			title = $(this).html(),
			div = $(this).closest('.aki_relationship'),
	...
	*/
	...
	$('.aki_relationship .relationship_right .relationship_list a').live('click', function(){
	...
	*/
	...
	$('.aki_relationship input.relationship_search').live('keyup', function()
	{	
		// vars
		var val = $(this).val(),
			div = $(this).closest('.aki_relationship');
	...
	_relationship.update_results = function( div ){
	    ...
	    $.ajax({
			...
			dataType: 'html',
			data: { 
				'action' : 'aki_get_relationship_results',
				's' : s,
				'paged' : paged,
				'taxonomy' : taxonomy,
				'post_type' : post_type,
