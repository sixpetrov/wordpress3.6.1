...
(function($){
	...
	var _validation = aki.validation;
	...
	*/
	...
	_validation.run = function(){
		...
		$('.postbox:not(.aki-hidden) .field.required, .form-field.required').each(function(){
			...
			if( div.hasClass('aki-conditional_logic-hide') )
			{
				return;
			}
			...
			// wysiwyg
			if( div.find('.aki_wysiwyg').exists() && typeof(tinyMCE) == "object")
			{
				div.data('validation', true);
			...
			// relationship
			if( div.find('.aki_relationship').exists() )
			{
				div.data('validation', false);
				
				if( div.find('.aki_relationship .relationship_right input').exists() )
				{
					div.data('validation', true);
				}
			}
			...
			// flexible content
			if( div.find('.aki_flexible_content').exists() )
			{
				div.data('validation', false);
				if( div.find('.aki_flexible_content .values table').exists() )
				{
					div.data('validation', true);
				}	
			}
			...
			// gallery
			if( div.find('.aki-gallery').exists() )
			{
				div.data('validation', false);
				
				if( div.find('.aki-gallery .thumbnail').exists())
				{
					div.data('validation', true);
				}
			}
			...
			$(document).trigger('aki/validate_field', div );
