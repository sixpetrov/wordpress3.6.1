...
(function($){
	...
	var _repeater = aki.fields.repeater;
	...
	*/
	...
	{
		// validate
		if( repeater.children('.aki-input-table').hasClass('row_layout') )
		{
			return;
		}
		...
		if( repeater.find('> .aki-input-table > thead > tr > th.order').exists() )
		{
			column_width = 93;
		}
		...
		repeater.find('> .aki-input-table  > thead > tr > th[width]').each(function( i ){
			
			column_width -= parseInt( $(this).attr('width') );
		});

		
		var ths = repeater.find('> .aki-input-table > thead > tr > th').not('[width]').has('span');
		if( ths.length > 1 )
		{
	...
	_repeater.add_sortable = function( repeater ){
		...
		repeater.find('> table > tbody').unbind('sortable').sortable({
			...
			start : function (event, ui) {
				...
				$(document).trigger('aki/sortable_start_repeater', ui.item);
   			...
   			stop : function (event, ui) {
				...
				$(document).trigger('aki/sortable_stop_repeater', ui.item);
	...
	*/
	...
	_repeater.update_classes = function( repeater )
	{
		...
		if( row_count >= max_rows )
		{
			...
			repeater.find('> .repeater-footer .aki-button').addClass('disabled');
		}
		else
		{
			repeater.removeClass('disabled');
			repeater.find('> .repeater-footer .aki-button').removeClass('disabled');
		}
	...
	*/
	...
	_repeater.add_row = function( repeater, before )
	{
		...
		var new_id = aki.helpers.uniqid(),
			new_field_html = repeater.find('> table > tbody > tr.row-clone').html().replace(/(=["]*[\w-\[\]]*?)(akicloneindex)/g, '$1' + new_id),
	...
	*/
	...
	$('.repeater td.remove .aki-button-remove').live('click', function(){
	...
	*/
	...
	$('.repeater tr').live('mouseenter', function(){
		...
		var button = $(this).find('> td.remove > a.aki-button-add');
