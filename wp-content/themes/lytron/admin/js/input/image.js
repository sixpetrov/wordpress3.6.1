...
(function($){
	...
	*/
	...
	_image.add = function( image )
	{
		...
		// set atts
		div.find('.aki-image-value').val( image.id ).trigger('change');
	...
	*/
	...
	_image.edit = function(){
		...
		// vars
		var div = _media.div,
			id = div.find('.aki-image-value').val();
		
		
		// show edit attachment
		tb_show( _image.text.title_edit , aki.admin_url + 'media.php?attachment_id=' + id + '&action=edit&aki_action=edit_attachment&aki_field=image&TB_iframe=1');
	...
	*/
	...
	_image.remove = function()
	{
		...
		// remove atts
		div.find('.aki-image-value').val('').trigger('change');
	...
	*/
	...
	$('.aki-image-uploader .add-image').live('click', function(){
				
		// vars
		var div = _media.div = $(this).closest('.aki-image-uploader'),
				...
				if( selection )
				{
					...
					selection.each(function(attachment){
				    	...
				    	if( i > 1 )
						{
							...
							{
								_media.div = tr.next('.row').find('.aki-image-uploader');
							}
							else
							{
								// add row 
				 				...
				 				_media.div = repeater.find('> table > tbody > tr.row:last .aki-image-uploader');
							}
						}
		...
		{
			tb_show( _image.text.title_add , aki.admin_url + 'media-upload.php?post_id=' + aki.post_id + '&post_ID=' + aki.post_id + '&type=image&aki_type=image&aki_preview_size=' + preview_size + 'TB_iframe=1');
		}
	...
	*/
	...
	$('.aki-image-uploader .aki-button-edit').live('click', function(){
		
		// vars
		_media.div = $(this).closest('.aki-image-uploader');
	...
	*/
	...
	$('.aki-image-uploader .aki-button-delete').live('click', function(){
