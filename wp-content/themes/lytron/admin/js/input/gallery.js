...
(function($){
	...
	var _gallery = aki.fields.gallery,
		_media = aki.media;
	...
	*/
	...
	_gallery.edit_image = function(){
		...
		tb_show( aki.fields.image.text.title_edit , aki.admin_url + 'media.php?attachment_id=' + id + '&action=edit&aki_action=edit_attachment&aki_field=gallery&TB_iframe=1');
	...
	*/
	...
	_gallery.update_image = function(){
		...
		// vars
		var div = aki.media.div,
			id = div.attr('data-id'),
			ajax_data = {
				action : 'aki/fields/gallery/get_metadata',
				id : id,
				nonce : aki.nonce
			};
	...
	*/
	...
	$('.aki-gallery .toolbar .view-grid').live('click', function(){
		
		// vars
		var gallery = $(this).closest('.aki-gallery');
	...
	*/
	...
	$('.aki-gallery .toolbar .view-list').live('click', function(){
	...
	*/
	...
	// add image
	$('.aki-gallery .toolbar .add-image').live('click', function(){
		
		// vars
		var div = _media.div = $(this).closest('.aki-gallery'),
		...
		if( _media.type() == 'backbone' )
		{
			...
			// aki.media.frame.content.get().options.filters=true;
			...
			_media.frame.on('content:create:browse', function( e ){
				...
				/*
aki.media.frame.content.get().collection.on( 'all', function( e ){
...
*/
			...
			_media.frame.on('content:activate:browse', function(){
				...
				aki.media.frame.content.get().collection.on( 'reset add', function(){
...
*/
			...
			aki.media.frame.on('content:activate:upload', function(){
				$(document).trigger('aki/media/upload');
			});
	
			// event fired when opening the media popup
			aki.media.frame.on('open', function( e ){
				
				aki.media.frame.content.get().attachments.render();
				aki.media.frame.reset();
				$(document).trigger('aki/media/open');
				
				
		    });
		    
		    aki.media.frame.on('reset', function( e ){
				$(document).trigger('aki/media/reset');
			...
			aki.media.frame.on('close', function( e ){
				
				// clear the div
				$(document).trigger('aki/media/close');
		    });
		    
		    
			// When an image is selected, run a callback.
		    aki.media.frame.on('select', function()
		    {
		    	$(document).trigger('aki/media/select', [aki.media.frame.state().get('selection')]);
		    }); */
		}
		else
		{
			tb_show( _gallery.text.title_add , aki.admin_url + 'media-upload.php?post_id=' + aki.post_id + '&post_ID=' + aki.post_id + '&type=image&aki_type=gallery&aki_preview_size=' + preview_size + 'TB_iframe=1');
		}
	...
	*/
	...
	$('.aki-gallery .thumbnail .aki-button-edit').live('click', function(){
	...
	*/
	...
	$('.aki-gallery .thumbnail .aki-button-delete').live('click', function(){
		
		// vars
		var thumbnail = $(this).closest('.thumbnail'),
			gallery = thumbnail.closest('.aki-gallery');
		...
		if( collection )
		{
			collection.each(function( item ){
				...
				if( gallery.find('.thumbnails .thumbnail[data-id="' + item.id + '"]').exists() )
				{
					...
					attachment.addClass('aki-selected');
				}
	...
	*/
	...
	$(document).live('aki/setup_fields', function(e, postbox){
		...
		$(postbox).find('.aki-gallery').each(function(i){
			
			// is clone field?
			if( aki.helpers.is_clone_field($(this).children('input[type="hidden"]')) )
			{
				return;
			}
