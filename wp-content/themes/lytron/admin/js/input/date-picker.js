...
(function($){
	...
	*/
	...
	$(document).live('aki/setup_fields', function(e, postbox){
		...
		$(postbox).find('input.aki_datepicker').each(function(){
			
			// vars
			var input = $(this),
				alt_field = input.siblings('.aki-hidden-datepicker'),
				save_format = input.attr('data-save_format'),
				display_format = input.attr('data-display_format');
			
			
			// is clone field?
			if( aki.helpers.is_clone_field(alt_field) )
			{
				return;
			}
			...
			{
				$('#ui-datepicker-div').wrap('<div class="ui-aki" />');
			}
